...
# (подводка пара от эстакады к стояку — см. секцию 9, после определения RY)
...
# СБРОСНОЙ ГАЗ (FlareGas): сепаратор → эстакада → врезка в ствол
# Маршрут идёт только до поверхности ствола — внутри ствола газ поднимается невидимо
route([
    ...
    (FX+0.7, FY,  FLARE_Z),         # 4: к стволу — врезка
    (FX,     FY,  FLARE_Z),         # 5: вход в ствол — joint в точке врезки
...
# Подводка пара от эстакады к паровому стояку (нижний конец SteamRise_L на z=0.5)
# Горизонтально от стояка → к линии эстакады → вверх до уровня эстакады
route([
    (SX0, SY0, 0.5),                # 0: низ стояка SteamRise_L
    (SX0, RY,  0.5),                # 1: к эстакаде по Y
    (SX0, RY,  SHOE_Z),             # 2: вверх до уровня опор эстакады — joint
], r=STEAM_PIPE_R*0.7, m=MS, seg=12, name="SteamFeed", joint_m=MS)
